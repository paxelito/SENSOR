...
def importAgents(tmpDir, tmpSeed):
	...
	try:
		...
		print "Agent file has not been found: ", tmpFileName; sys.exit(1)
	fileFID = open(tmpFileName, 'r')
	#read file
...
def importAgentTechList(tmpDir, tmpSeed):
	...
	try:
		...
		print "Agent Tech file has not been found: ", tmpFileName; sys.exit(1)
	fileFID = open(tmpFileName, 'r')
	#read file
			...
			for sngRow in agentTechs:
				if sngRow[0] != '#':
					...
					if cnt == 0:
						...
						finalMtx = np.vstack([finalMtx,(agentMtx[agentID,0],agentMtx[agentID,1],agentMtx[agentID,2],agentMtx[agentID,3],agentMtx[agentID,4],\
											tech, prop, age)])
					cnt += 1
